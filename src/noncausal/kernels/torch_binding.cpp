// PyTorch bindings for the non-causal RACE kernels (race_fwd.cu, race_bwd.cu).
//
//   forward(q, k, v, W, beta) -> o
//   forward_debug(q, k, v, W, beta) -> (o, A, B)
//   forward_train(q, k, v, W, beta) -> (o, bucket_totals)
...
// The widest global access in the kernels is 8 bytes per lane (4 bf16).
...
std::tuple<at::Tensor, at::Tensor> run_forward(const at::Tensor& q, const at::Tensor& k,
    ...
    C10_CUDA_CHECK(race::race_forward(bf16_ptr(q), bf16_ptr(k), bf16_ptr(v),
                                      W.data_ptr<float>(), beta_device.data_ptr<float>(),
                                      workspace.data_ptr<float>(), bf16_ptr(out), shape, stream));
...
at::Tensor forward(const at::Tensor& q, const at::Tensor& k, const at::Tensor& v,
    ...
    return std::get<0>(run_forward(q, k, v, W, beta, shape));
}
...
std::tuple<at::Tensor, at::Tensor> forward_train(const at::Tensor& q, const at::Tensor& k,
                                                 ...
                                                 const at::Tensor& beta) {
    const race::ForwardShape shape = validate_inputs(q, k, v, W, beta);
    auto [out, workspace] = run_forward(q, k, v, W, beta, shape);
