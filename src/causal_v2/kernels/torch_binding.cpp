// PyTorch bindings for the causal RACE v2a forward kernels (race_causal_fwd.cu).
//
//   forward(q, k, v, W, beta, tile_tokens=0) -> o
//   forward_debug(q, k, v, W, beta, tile_tokens=0)
//       -> (o, prefix_A, prefix_B, final_A, final_B, tile_tokens)
//   sub_chunk_tokens(head_dim, num_planes) -> C
//   select_tile_tokens(batch_heads, seq_len, head_dim, num_planes, num_tables) -> T_blk
//   output_pass_smem_bytes(head_dim, num_planes, num_tables) -> bytes
//   fits_on_device(head_dim, num_planes, num_tables) -> bool
//
...
void check_fits_device(const race::causal::CausalShape& shape) {
    bool fits = false;
    int limit_bytes = 0;
    C10_CUDA_CHECK(race::causal::race_causal_output_fits(shape, &fits, &limit_bytes));
    TORCH_CHECK(fits, "the causal output pass needs ",
                race::causal::output_pass_smem_bytes(shape), " bytes of shared memory per block for d=",
                shape.head_dim, ", P=", shape.num_planes, ", L=", shape.num_tables,
                "; this GPU allows ", limit_bytes);
}

int resolve_tile_tokens(const race::causal::CausalShape& shape, int64_t requested) {
    if (requested == 0) {
        int tile_tokens = 0;
        C10_CUDA_CHECK(race::causal::race_causal_select_tile_tokens(shape, &tile_tokens));
        return tile_tokens;
    }
    TORCH_CHECK(requested > 0 && requested <= std::numeric_limits<int>::max() &&
                    race::causal::is_valid_tile_tokens(shape, static_cast<int>(requested)),
                "tile_tokens must be a positive multiple of ",
                race::causal::sub_chunk_tokens(shape.head_dim, shape.num_planes), ", got ",
                requested);
...
ForwardResult run_forward(const at::Tensor& q, const at::Tensor& k, const at::Tensor& v,
                          ...
                          bool want_final_state) {
    const c10::cuda::CUDAGuard device_guard(q.device());
    ForwardResult result;
    result.out = at::empty_like(q);
    if (shape.seq_len == 0 || shape.batch_heads == 0) return result;

    check_fits_device(shape);
    result.tile_tokens = resolve_tile_tokens(shape, requested_tile_tokens);
...
at::Tensor forward(const at::Tensor& q, const at::Tensor& k, const at::Tensor& v,
    ...
    return run_forward(q, k, v, W, beta, shape, tile_tokens, false).out;
}
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, int64_t> forward_debug(
    ...
    const at::Tensor& beta, int64_t tile_tokens) {
    const race::causal::CausalShape shape = validate_inputs(q, k, v, W, beta);
    ForwardResult result = run_forward(q, k, v, W, beta, shape, tile_tokens, true);
...
int64_t sub_chunk_tokens(int64_t head_dim, int64_t num_planes) {
...
int64_t select_tile_tokens(int64_t batch_heads, int64_t seq_len, int64_t head_dim,
    ...
    check_fits_device(shape);
    return resolve_tile_tokens(shape, 0);
}

int64_t output_pass_smem_bytes(int64_t head_dim, int64_t num_planes, int64_t num_tables) {
    return static_cast<int64_t>(
        race::causal::output_pass_smem_bytes(make_shape(1, 1, head_dim, num_planes, num_tables)));
}

bool fits_on_device(int64_t head_dim, int64_t num_planes, int64_t num_tables) {
    bool fits = false;
    int limit_bytes = 0;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("sub_chunk_tokens", &sub_chunk_tokens, "Sub-chunk length C of the output pass",
          py::arg("head_dim"), py::arg("num_planes"));
    ...
    m.def("output_pass_smem_bytes", &output_pass_smem_bytes,
          ...
          py::arg("num_planes"), py::arg("num_tables"));
